...
setwd("~/projects/PhoCl/barrel/analysis/cedar_ASMD")
...
p <- ggplot(data = dat_post, aes(x = Time, y = RMSD)) +
  ...
  scale_x_continuous(breaks=seq(0, 400, 200), limits=c(-40,430), expand=c(0,0)) +
  # Specify the breaks of x-axis
  geom_segment(y=0, yend=10, x=-40, xend=-40, lwd=0.5, colour="black", lineend="square") +
  # Specify the location of your new y-axis
  geom_segment(y=-0.2, yend=-0.2, x=0, xend=400, lwd=0.5, colour="black", lineend="square")
...
p <- ggplot(data = dat_post, aes(x = Time, y = RMSD)) +
  ...
  scale_y_continuous(breaks=seq(0,10,5), limits=c(-0.2,10.2), expand=c(0,0)) +
  # Specify the breaks of y-axis
  scale_x_continuous(breaks=seq(0, 20, 5), limits=c(-2,22), expand=c(0,0)) +
  # Specify the breaks of x-axis
  geom_segment(y=0, yend=10, x=-2, xend=-2, lwd=0.5, colour="black", lineend="square") +
  # Specify the location of your new y-axis
  geom_segment(y=-0.2, yend=-0.2, x=0, xend=20, lwd=0.5, colour="black", lineend="square")
